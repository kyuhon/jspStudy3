...
function idCheck() {
	...
	if (value === "") {
		...
		url = "check/idCheck.jsp?id=" + value;
		window.open(url, "post", "width=500,height=300");
	}
}
/*회원가입폼 주소체크*/
function zipCheck() {
	url = "check/zipCheck.jsp?check=y";
...
function sendAddress(zipcode, sido, gugun, dong, bunji) {
	...
	opener.document.signUp.zipcode.value = zipcode;
	opener.document.signUp.address1.value = address;
	self.close();
}
/*회원가입폼 패턴검색*/
function inputCheck() {
	...
	if (document.signUp.phone3.value == "") {
		alert("전화번호을 입력해 주세요.");
		document.signUp.phone3.focus();
		return;
	}
	...
	if (document.signUp.address2.value == "") {
		alert("세부주소를 입력해 주세요.");
		document.signUp.address2.focus();
		return;
	}
